...
import { InformationResponse } from "./libs/types/types.js";
...
const InformationSection = () => {
	...
	<div className={styles.container}>
			<picture className={styles.imgBlock}>
				<img className={styles.img} src={info?.thumbnailLink} alt={info?.title} />
			</picture>
			<div className={styles.textBlock}>
				<h2 className={styles.title}>{info?.title}</h2>
				<div className={styles.tagBlock}>
					{ info?.tags.map((item, index) => <span className={styles.tag} key={index}>#{item.title}</span>)}
				</div>
